...
| Wipe-cycle "next wipe" estimate | Rust+ reports only the last wipe timestamp, never a schedule; the cycle length (weekly/biweekly/monthly) is entirely user-entered and the UI always labels the countdown "(your estimate)" | Not resolvable from Rust+ alone — always a user-supplied convention, not evidence to gather |
